...
if (-not ($healthResult.Success -and $healthResult.StatusCode -eq 200)) {
    ...
    Write-Host "2. Navigate to backend directory: cd E:\esportsPlatform\esports-platform-monorepo\backend" -ForegroundColor White
    Write-Host "3. Run: npm run dev" -ForegroundColor White
    Write-Host "4. Wait for server to start, then run this script again" -ForegroundColor White
    exit 1
}
...
Write-Host "`n🪝 MODULE 1: WEBHOOK TESTING (Payment Flow)" -ForegroundColor Cyan
Write-Host "=============================================" -ForegroundColor Cyan
...
Write-Host "`n📊 MODULE 2: DASHBOARD TESTING (Organizer Overview)" -ForegroundColor Cyan
Write-Host "===================================================" -ForegroundColor Cyan
...
Write-Host "`n🏆 MODULE 3: BRACKET TESTING (Match Generation & Retrieval)" -ForegroundColor Cyan
Write-Host "==========================================================" -ForegroundColor Cyan
...
Write-Host "`n📊 FOCUSED UAT RESULTS SUMMARY" -ForegroundColor Magenta
...
foreach ($module in $testResults.Keys) {
    ...
    $status = if ($percentage -eq 100) { "✅" } elseif ($percentage -ge 80) { "⚠️" } else { "❌" }
    Write-Host "`n$status $module Module: $passed/$total tests passed ($percentage%)" -ForegroundColor $(if ($percentage -eq 100) { "Green" } elseif ($percentage -ge 80) { "Yellow" } else { "Red" })
    
    # Show detailed results
    foreach ($test in $tests) {
        $testStatus = if ($test.Pass) { "✅" } else { "❌" }
        Write-Host "   $testStatus $($test.Test): $($test.Actual)" -ForegroundColor $(if ($test.Pass) { "Green" } else { "Red" })
...
Write-Host "`n🎯 UAT CONFIDENCE SCORE" -ForegroundColor Cyan
Write-Host "========================" -ForegroundColor Cyan
...
if ($overallPercentage -eq 100) {
    Write-Host "`n🏆 CONFIDENCE SCORE: EXCELLENT (100%)" -ForegroundColor Green
    ...
    Write-Host "`n✨ CONFIDENCE SCORE: HIGH ($overallPercentage%)" -ForegroundColor Yellow
    ...
    Write-Host "`n⚠️  CONFIDENCE SCORE: MODERATE ($overallPercentage%)" -ForegroundColor Yellow  
    ...
    Write-Host "`n🔧 CONFIDENCE SCORE: LOW ($overallPercentage%)" -ForegroundColor Red
    Write-Host "   Significant issues detected. Deployment not recommended." -ForegroundColor Red
} else {
    Write-Host "`n🚨 CONFIDENCE SCORE: CRITICAL ($overallPercentage%)" -ForegroundColor Red
    Write-Host "   Major failures detected. Immediate attention required." -ForegroundColor Red
}

Write-Host "`n📄 Focused UAT Testing Complete!" -ForegroundColor Green
...
$resultsSummary | ConvertTo-Json -Depth 10 | Out-File "focused-uat-results.json" -Encoding UTF8
Write-Host "`n💾 Results saved to: focused-uat-results.json" -ForegroundColor Blue